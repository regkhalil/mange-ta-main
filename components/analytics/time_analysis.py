...
def get_time_categories(df: pd.DataFrame) -> pd.DataFrame:
    """
    Use precomputed time categories from preprocessing.

    Note: This function now returns the dataframe as-is since time_category
    is precomputed during preprocessing.

    Args:
        df: Recipe dataframe with precomputed time_category column

    Returns:
        DataFrame (unchanged, kept for backward compatibility)
    """
    if "time_category" not in df.columns:
        logger.error("Precomputed time_category column not found in dataframe")
        logger.error(f"Available columns: {df.columns.tolist()}")
    else:
        logger.info(f"Using precomputed time categories for {len(df)} recipes")
...
def analyze_time_vs_score(df: pd.DataFrame) -> dict:
    ...
    clean_df = clean_df[
        (clean_df["minutes"] > 0) & (clean_df["minutes"] < 500)  # Remove extreme outliers
    ]
    ...
    logger.info(
        f"Correlation results - Pearson: r={pearson_r:.3f} (p={pearson_p:.3e}), Spearman: ρ={spearman_r:.3f} (p={spearman_p:.3e})"
    )
...
def create_time_scatter(df: pd.DataFrame) -> go.Figure:
    ...
    plot_df = plot_df[
        (plot_df["minutes"] > 0) & (plot_df["minutes"] < 300)  # Limit to 5 hours for better viz
    ]
...
def create_grade_by_time_category(df: pd.DataFrame) -> go.Figure:
    ...
    category_order = ["Rapide (≤15min)", "Moyen (15-30min)", "Long (30-60min)", "Très long (>60min)"]
