...
def filter_outliers_percentile(
    df: pd.DataFrame, columns: list, lower_pct: float = 0.01, upper_pct: float = 0.99
) -> pd.DataFrame:
    """
...
def sample_large_dataset(df: pd.DataFrame, max_size: int = 5000, random_state: int = 42) -> pd.DataFrame:
    """
