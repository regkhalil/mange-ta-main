...
def calculate_complexity_index(df: pd.DataFrame) -> pd.DataFrame:
    """
    Use precomputed complexity index from preprocessing.

    Note: This function now returns the dataframe as-is since complexity_index
    and complexity_category are precomputed during preprocessing.

    Args:
        df: Recipe dataframe with precomputed complexity_index and complexity_category columns

    Returns:
        DataFrame (unchanged, kept for backward compatibility)
    """
    logger.info(f"Using precomputed complexity index for {len(df)} recipes")

    if "complexity_index" not in df.columns or "complexity_category" not in df.columns:
        logger.error("Precomputed complexity columns not found in dataframe")
        logger.error(f"Available columns: {df.columns.tolist()}")
    else:
        logger.info(f"Complexity index range: {df['complexity_index'].min():.1f}-{df['complexity_index'].max():.1f}")
...
def analyze_complexity_vs_health(df: pd.DataFrame) -> dict:
    ...
    category_stats = df_complex.groupby("complexity_category")["nutrition_score"].agg(
        ["mean", "median", "std", "count"]
    )

    return {"correlations": correlations, "category_stats": category_stats, "n_samples": len(clean_df)}
...
def create_complexity_scatter(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(x=x_trend, y=y_trend, mode="lines", name="Tendance", line=dict(color="red", width=2, dash="dash"))
    )
...
def create_complexity_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    df_binned = df.copy()
    df_binned = df_binned[(df_binned["n_steps"] <= 30) & (df_binned["n_ingredients"] <= 20)].dropna(
        subset=["n_steps", "n_ingredients", "nutrition_score"]
    )

    # Create bins
    df_binned["steps_bin"] = pd.cut(
        df_binned["n_steps"], bins=[0, 5, 10, 15, 20, 30], labels=["1-5", "6-10", "11-15", "16-20", "21-30"]
    )
    df_binned["ingredients_bin"] = pd.cut(
        df_binned["n_ingredients"], bins=[0, 5, 10, 15, 20], labels=["1-5", "6-10", "11-15", "16-20"]
    )

    # Calculate average score for each bin combination
    heatmap_data = (
        df_binned.groupby(["steps_bin", "ingredients_bin"])["nutrition_score"].mean().unstack(fill_value=np.nan)
    )
...
def create_individual_factors_chart(df: pd.DataFrame) -> go.Figure:
    """
    Create simple bar chart showing correlation of each complexity factor.
    ...
    # Color based on correlation direction: positive = green, negative = red
    colors = ["#238B45" if c > 0 else "#E63946" for c in corr_values]
    ...
    fig.update_layout(
        title="Corrélation de Chaque Facteur avec le Score Nutritionnel<br><sub>Vert = corrélation positive, Rouge = corrélation négative</sub>",
