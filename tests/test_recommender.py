...
class TestRecommender:
    ...
    def sample_recipes(self):
        ...
        return pd.DataFrame(
            {
                "id": [1, 2, 3, 4, 5],
                "name": ["Chocolate Cake", "Vanilla Cake", "Chocolate Cookies", "Strawberry Pie", "Apple Pie"],
                "ingredients": [
    ...
    def test_get_similar_recipes_excludes_input(self, sample_recipes):
        ...
        try:
            ...
            if len(similar) > 0:
                similar_ids = [
                    recipe["id"] if isinstance(recipe, pd.Series) else recipe.get("id", None) for recipe, _ in similar
                ]
