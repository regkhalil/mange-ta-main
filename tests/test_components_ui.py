...
class TestMetricsHeader:
    """Tests pour l'en-tête de métriques."""
    ...
    def test_render_metrics_header_missing_stats(self, mock_st):
        """Test du rendu des métriques avec stats manquantes"""
        # Mock columns
        col1_mock = Mock()
        col2_mock = Mock()
        col3_mock = Mock()
        ...
        # Vérifier les appels metric avec valeurs par défaut
        col1_mock.metric.assert_called_once_with(label="📚 Recettes totales", value="500")
        col2_mock.metric.assert_called_once_with(label="⏱️ Temps médian", value="0 min")
        col3_mock.metric.assert_called_once_with(label="🔥 Calories moyennes", value="300 kcal")
        col4_mock.metric.assert_called_once_with(label="🌱 Végétarien", value="0.0%")

    @patch('components.metrics_header.st')
    def test_render_metrics_header_empty_stats(self, mock_st):
        """Test du rendu des métriques avec stats vides"""
        # Mock columns
        col1_mock = Mock()
        col2_mock = Mock()
        col3_mock = Mock()
        ...
        # Vérifier les appels metric avec valeurs par défaut
        col1_mock.metric.assert_called_once_with(label="📚 Recettes totales", value="0")
        col2_mock.metric.assert_called_once_with(label="⏱️ Temps médian", value="0 min")
        col3_mock.metric.assert_called_once_with(label="🔥 Calories moyennes", value="0 kcal")
        col4_mock.metric.assert_called_once_with(label="🌱 Végétarien", value="0.0%")
...
class TestNutriScore:
    ...
    def test_nutri_score_with_real_data(self):
        ...
        df = pd.DataFrame({
            "nutrition_score": [88.5, 72.3, 65.1, 48.7, 31.2, None, 95.0, 15.5]
        })
