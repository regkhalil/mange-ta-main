...
class TestAnalyticsIntegration:
    ...
    def test_comprehensive_analytics_workflow(self):
        ...
        df["complexity_score"] = (df["n_steps"] * 0.3 + df["n_ingredients"] * 0.2 + df["minutes"] * 0.005).clip(0, 100)

        # Étape 3: Catégorisation
        df["time_category"] = pd.cut(df["minutes"], bins=[0, 30, 60, 180], labels=["Quick", "Medium", "Long"])
...
class TestAnalyticsEdgeCases:
    ...
    def test_single_row_analytics(self):
        ...
        single_row_df = pd.DataFrame({"value": [42], "category": ["A"]})
    ...
    def test_missing_values_analytics(self):
        ...
        df_with_na = pd.DataFrame({"value": [1, 2, None, 4, 5], "category": ["A", "B", "A", None, "B"]})
    ...
    def test_extreme_values_analytics(self):
        ...
        extreme_df = pd.DataFrame(
            {"value": [0, 1e6, -1000, 0.001, float("inf")], "category": ["A", "B", "C", "D", "E"]}
        )
