...
class TestModuleIntegration:
    ...
    def test_extract_nutrition_scoring_consistency(self, sample_raw_data):
        ...
        pd.testing.assert_series_equal(
            df1_scored['nutrition_score'],
            df2_scored['nutrition_score']
        )
...
class TestRealWorldScenarios:
    ...
    def test_dietary_preferences_analysis(self):
        ...
        vegan_score = df[df['name'].str.contains('vegan', case=False)]['nutrition_score'].iloc[0]
        low_sodium_score = df[df['name'].str.contains('low sodium', case=False)]['nutrition_score'].iloc[0]
        high_protein_score = df[df['name'].str.contains('high protein', case=False)]['nutrition_score'].iloc[0]
...
class TestErrorHandlingAndRobustness:
    ...
    def test_memory_efficiency_large_dataset(self):
        ...
        base_data = {
            ...
            'nutrition': [
                [200 + (i % 400), 10 + (i % 20), 5 + (i % 15), 
                 300 + (i % 700), 15 + (i % 30), 3 + (i % 12), 25 + (i % 50)]
