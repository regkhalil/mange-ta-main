...
class TestAnalyticsImports(unittest.TestCase):
    ...
    def test_nutrition_profiling_import(self):
        ...
        try:
            from components.analytics.nutrition_profiling import analyze_nutrition_trends, create_nutrition_profile
...
class TestAnalyticsIntegration(unittest.TestCase):
    ...
    def test_analytics_dependencies_handling(self):
        ...
        try:
            # Test que le package analytics existe au moins
            import importlib.util

            spec = importlib.util.find_spec("components.analytics")
            if spec is not None:
                self.assertTrue(True)
            else:
                self.skipTest("components.analytics package not found")
        except ImportError as e:
    ...
    def test_analytics_with_minimal_data(self):
        ...
        minimal_df = pd.DataFrame(
            {"id": [1, 2, 3], "name": ["Recipe A", "Recipe B", "Recipe C"], "nutrition_score": [80, 60, 40]}
        )
