...
from services.data_loader import filter_recipes, get_recipe_stats, load_recipes
from services.pexels_image_service import get_image_with_fallback
from services.recommender import RecipeRecommender, get_recommender
...
class TestServicesIntegration:
    ...
    def test_search_and_filter_integration(self, recipes_data):
        ...
        veg_results, veg_total = search_recipes(
            recipes_data,
    ...
    def test_filter_and_recommend_workflow(self, recipes_data):
        ...
        vegetarian_recipes = filter_recipes(
            recipes_data,
            vegetarian_only=True,
            prep_range=(10, 60)
        )
        ...
        if len(vegetarian_recipes) > 0:
            ...
            with patch('services.recommender.read_pickle_file') as mock_read_pickle:
                ...
                mock_similarity_data = {
                    ...
                    "combined_features": np.array([[0.1 * i, 0.2 * i, 0.3 * i] for i in range(len(vegetarian_recipes))])
                }
                mock_read_pickle.return_value = mock_similarity_data
                ...
                for recipe, score in similar_recipes:
                    assert recipe["is_vegetarian"] == True  # Utiliser == au lieu de is
    ...
    def test_search_with_images_integration(self, mock_get, recipes_data):
        ...
        mock_response.json.return_value = {
            "photos": [{
                "src": {"large": "https://images.pexels.com/test-image.jpg"}
            }]
        }
    ...
    def test_performance_integration(self, recipes_data):
        ...
        filtered_recipes = filter_recipes(
            ...
            calories_range=(100, 600)
        )
        
        if len(filtered_recipes) > 0:
            # Recherche dans les données filtrées
            search_results, total = search_recipes(
                filtered_recipes,
                query="recipe",
                sort_by="prep_time",
                page_size=10
            ...
            stats = get_recipe_stats(filtered_recipes)
