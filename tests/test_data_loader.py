...
class TestPerformance:
    ...
    def test_load_interactions_reasonable_time(self, data_dir):
        ...
        try:
            read_csv_file("interactions_train.csv", data_dir=data_dir)
        except Exception:
            pd.DataFrame()
        duration = time.time() - start
    ...
    def test_partial_load_faster(self, data_dir):
        ...
        # Chargement partiel
        start = time.time()
        read_csv_file("preprocessed_recipes.csv", data_dir=data_dir, nrows=100)
        time_partial = time.time() - start

        # Chargement complet
        start = time.time()
        read_csv_file("preprocessed_recipes.csv", data_dir=data_dir)
...
class TestEdgeCases:
    ...
    def test_vegetarian_recipes_exist(self, recipes_df):
        ...
        vegetarian = recipes_df[recipes_df["is_vegetarian"]]
