...
def prepare_vegetarian_classification(df: pd.DataFrame) -> pd.DataFrame:
    ...
    non_veg_keywords = [
        # Meat
        'beef', 'pork', 'lamb', 'veal', 'venison', 'mutton', 'goat',
        # Poultry
        ...
        'pancetta', 'mortadella', 'bratwurst', 'kielbasa', 'hot dog', 'frankfurter',
        # Ground meats
        'ground beef', 'ground pork', 'ground turkey', 'ground chicken', 'ground lamb',
        # Other animal products
        'gelatin', 'lard', 'tallow', 'suet', 'bone', 'marrow',
        # Broths and stocks
        'chicken broth', 'beef broth', 'chicken stock', 'beef stock', 'bone broth',
        # General terms
        'meat', 'steak', 'roast', 'cutlet', 'fillet', 'wing', 'thigh', 'breast'
    ]
    ...
    logging.info(f"Classification complete: {vegetarian_count}/{total_count} recipes classified as vegetarian ({vegetarian_count/total_count*100:.1f}%)")
...
def filter_vegetarian_recipes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    vegetarian_df = df[df['is_vegetarian'] == True].copy()
